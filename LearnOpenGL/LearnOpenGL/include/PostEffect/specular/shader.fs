...
{ 
    if (PostEffect == 0)
    {
        FragColor = vec4(vec3(texture(scene, TexCoords).a), 1.0);
    }
    else if (PostEffect == 1) // bloom effect
    {
        ...
        FragColor = 0.25 * blur2 + 0.5 * blur + vec4(vec3(texture(scene, TexCoords).a), 1.0);
    }
    else if (PostEffect == 2)
    {
        vec3 value = vec4(vec3(texture(scene, TexCoords).a), 1.0).rgb;
