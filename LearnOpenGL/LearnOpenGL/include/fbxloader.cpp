...
vector<attrib_t> gAttribs;
vector<attrib_t> gAttribAnim;
...
bool LoadFbx(fbx_handles &handles, vector<attrib_t> &attribs, vector<shape_t> &shapes, vector<material_t> &materials, std::string err, const char* pFileName)
{
	gAttribs.clear();
	gShapes.clear();
	gMaterials.clear();
    ...
    if (lNodeAttribute)
    {
        ...
        if (lNodeAttribute->GetAttributeType() == FbxNodeAttribute::eMesh)
        {
            ...
            if (lMesh)
            {
				...
				attrib_t attrib;
				attrib.vertices = lVertices;
				gAttribAnim.push_back(attrib);
    ...
    if (lNodeAttribute)
    {
        ...
        if (lNodeAttribute->GetAttributeType() == FbxNodeAttribute::eMesh)
        {
            ...
            if (lMesh)
            {
				...
				{
					index_t index;
					index.vertex_index = lIndices[i];
					shape.mesh.indices.push_back(index);
					shape.mesh.material_ids = lMaterialIndices;
				}
				attrib_t attrib;
				attrib.vertices = lVertices;
				attrib.normals = lNormals;
				attrib.texcoords = lUVs;
				gShapes.push_back(shape);
				gAttribs.push_back(attrib);
